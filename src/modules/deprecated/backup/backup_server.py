...
class BackupClient:
    ...
    def backup_main(user_did, client):
        try:
            clog().info(f'[backup_main] enter backup thread, {user_did}, {client}.')
            client.backup(user_did)
        except Exception as e:
            clog().error(f'Failed to backup really: {traceback.format_exc()}')
            client.delete_mongodb_data(user_did)
            client.update_backup_state(user_did, VAULT_BACKUP_STATE_STOP, VAULT_BACKUP_MSG_FAILED)
        clog().info('[backup_main] leave backup thread.')

    @staticmethod
    def restore_main(user_did, client):
        try:
            clog().info(f'[restore_main] enter restore thread, {user_did}, {client}.')
    ...
    def backup(self, user_did):
        ...
        doc = cli.find_one_origin(DID_INFO_DB_NAME, VAULT_BACKUP_INFO_COL, {USER_DID: user_did})
        ...
        if self.is_ipfs:
            ...
            self.backup_ipfs_upload_dbfiles(user_did, doc[VAULT_BACKUP_INFO_DRIVE], doc[VAULT_BACKUP_INFO_TOKEN])
            clog().info('[backup_main: ipfs] success to upload database files.')
            self.backup_ipfs_cids(user_did, doc[VAULT_BACKUP_INFO_DRIVE], doc[VAULT_BACKUP_INFO_TOKEN])
        ...
        self.update_backup_state(user_did, VAULT_BACKUP_STATE_STOP, VAULT_BACKUP_MSG_SUCCESS)
    ...
    def restore(self, user_did):
        ...
        doc = cli.find_one_origin(DID_INFO_DB_NAME, VAULT_BACKUP_INFO_COL, {USER_DID: user_did})
        if self.is_ipfs:
            self.restore_ipfs_download_dbfiles(user_did, doc[VAULT_BACKUP_INFO_DRIVE], doc[VAULT_BACKUP_INFO_TOKEN])
            ...
            self.restore_finish(user_did, doc[VAULT_BACKUP_INFO_DRIVE], doc[VAULT_BACKUP_INFO_TOKEN])
...
class BackupServer:
    ...
    def _create_backup(self, user_did, price_plan):
        ...
        doc = {VAULT_BACKUP_SERVICE_DID: user_did,
    ...
    def backup_files(self):
        ...
        return {'backup_files': fm.get_file_checksum_list(get_vault_backup_path(user_did))}
    ...
    def backup_get_file(self, file_name):
        ...
        user_did, _, _ = self._check_auth_backup()
        return self.http_server.create_range_request((get_vault_backup_path(user_did) / file_name).resolve())
    ...
    def backup_upload_file(self, file_name):
        ...
        user_did, _, backup = self._check_auth_backup(is_check_size=is_ipfs)
        dst_file = (get_vault_backup_path(user_did) / file_name).resolve()
    ...
    def backup_delete_file(self, file_name):
        ...
        user_did, _, _ = self._check_auth_backup()
        fm.delete_vault_file(user_did, file_name)
    ...
    def backup_get_file_hash(self, file_name):
        ...
        user_did, _, _ = self._check_auth_backup()
        return fm.get_hashes_by_file((get_vault_backup_path(user_did) / file_name).resolve())
    ...
    def backup_patch_file(self, file_name):
        ...
        temp_file.unlink()
        fm.apply_rsync_data((get_vault_backup_path(user_did) / file_name).resolve(), pickle_data)
    ...
    def ipfs_backup_state(self, to, vault_size):
        ...
        user_did, _, backup = self._check_auth_backup()
        ...
        self.ipfs_update_state_really(user_did, to, vault_size)

    def ipfs_update_state_really(self, user_did, to, vault_size=0):
        ...
        cli.update_one_origin(DID_INFO_DB_NAME, VAULT_BACKUP_SERVICE_COL, {DID: user_did}, update)

    @hive_restful_response
    def ipfs_pin_cids(self, total_size, cids):
        user_did, _, backup = self._check_auth_backup(is_check_size=True)
    ...
    def ipfs_promotion(self):
        ...
        vault_subscription.create_vault(user_did, vault_subscription.get_price_plan('vault', 'Free'), True)
