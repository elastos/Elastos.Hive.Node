...
class HiveScripting:
    ...
    def set_subcondition(self):
        ...
        if schema_db is None:
            add_did_resource_to_db(did, app_id, collection, json.dumps(SCRIPTING_SUBCONDITIONS_SCHEMA))

        # Insert/Update the data into the database
        db_name = gene_mongo_db_name(did, app_id)
        upsert_subcondition_to_db(did, app_id, db_name, collection, name, condition)
    ...
    def set_script(self):
        ...
        schema_db = find_schema_of_did_resource(did, app_id, collection)
        if schema_db is None:
            add_did_resource_to_db(did, app_id, collection, json.dumps(SCRIPTING_SCRIPTS_SCHEMA))
    ...
    def run_script(self):
        ...
        db_name = gene_mongo_db_name(did, app_id)
        script = find_script_from_db(did, app_id, db_name, name)
        if script["error"]:
            return response_err(406, f"could not find the script '{name}'. Please register the script first")
