...
class Files:
    ...
    def _get_file_full_path(self, user_did, app_did, path):
        fixed_path = filter_path_root(path)
        return (get_save_files_path(user_did, app_did) / fixed_path).resolve()
    ...
    def upload_file_by_did(self, user_did, app_did, path):
    ...
    def download_file_by_did(self, user_did, app_did, path):
        data, status_code = query_download(user_did, app_did, path)
    ...
    def get_file_stat_by_did(self, user_did, app_did, path):
        full_path = self._get_file_full_path(user_did, app_did, path)
