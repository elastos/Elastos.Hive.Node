...
class HiveScripting:
    ...
    def __executable_execution(self, did, app_id, executable, params, output={}, output_key=None, capture_output=False):
        executable_type = executable.get('type')
        executable_body = executable.get('body')
        if executable_type == SCRIPTING_EXECUTABLE_TYPE_AGGREGATED:
            err_message = None
            for i, e in enumerate(executable_body):
                self.__executable_execution(did, app_id, e, params, output, e.get('name'), e.get('output', False))
        elif executable_type == SCRIPTING_EXECUTABLE_TYPE_FIND:
            data, err_message = run_executable_find(did, app_id, executable_body, params)
        elif executable_type == SCRIPTING_EXECUTABLE_TYPE_INSERT:
            data, err_message = run_executable_insert(did, app_id, executable_body, params)
        elif executable_type == SCRIPTING_EXECUTABLE_TYPE_UPDATE:
            data, err_message = run_executable_update(did, app_id, executable_body, params)
        elif executable_type == SCRIPTING_EXECUTABLE_TYPE_DELETE:
            data, err_message = run_executable_delete(did, app_id, executable_body, params)
        elif executable_type == SCRIPTING_EXECUTABLE_TYPE_UPLOAD:
            data, err_message = None, "cannot handle this type yet"
        elif executable_type == SCRIPTING_EXECUTABLE_TYPE_DOWNLOAD:
            data, err_message = run_executable_download(did, app_id, executable_body, params)
        else:
    ...
    def run_script(self):
        ...
        output = {}
        data = self.__executable_execution(did, app_id, executable, params, output=output)
