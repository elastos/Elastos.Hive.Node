...
class UpgradeChecker(unittest.TestCase):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        end_time = RA(response).body().get('end_time', int)
        self.assertEqual(pricing_plan, 'Standard')
...
class PaymentTestCase(unittest.TestCase):
    ...
    def test02_place_order(self):
        ...
        def assert_info(info):
            ...
            info.assert_equal('pricing_plan', 'Standard')
        ...
        response = self.cli.put('/order', body={'subscription': subscription, 'pricing_name': 'Standard'})
        RA(response).assert_status(200)
