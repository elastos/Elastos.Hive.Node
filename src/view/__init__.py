...
class RetryIpfsBackupThread(threading.Thread):
    ...
    def retry_ipfs_backup(self):
        ...
        for user_did in user_dids:
            ...
            self.retry_backup_request(user_did)
...
def init_app(app: Flask, api: Api):
    ...
    # auth service
    api.add_resource(auth.SignIn, URL_SIGN_IN, endpoint='auth.sign_in')
    api.add_resource(auth.Auth, URL_AUTH, endpoint='auth.auth')
    api.add_resource(auth.BackupAuth, URL_BACKUP_AUTH, endpoint='auth.backup_auth')
