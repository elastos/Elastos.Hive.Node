...
class FilesTestCase(unittest.TestCase):
    ...
    def test03_move_file(self):
        response = self.cli.patch(f'/files-deprecated/{self.src_file_name}?to={self.dst_file_name}')
        self.assertEqual(response.status_code, 200)
    ...
    def test04_copy_file(self):
        response = self.cli.put(f'/files-deprecated/{self.dst_file_name}?dest={self.src_file_name}')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json().get('name'), self.src_file_name)
        self.__check_remote_file_exist(self.dst_file_name)
        self.__check_remote_file_exist(self.src_file_name)
    ...
    def test05_list_folder(self):
        response = self.cli.get(f'/files-deprecated/{self.folder_name}?comp=children')
        self.assertEqual(response.status_code, 200)
    ...
    def test07_get_hash(self):
        response = self.cli.get(f'/files-deprecated/{self.src_file_name}?comp=hash')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json().get('name'), self.src_file_name)
